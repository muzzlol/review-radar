...
def test_model(reviews_list,threshold = 0.70):
    ...
    with open('../../models/fake_review_model.joblib', 'rb') as file:
    ...
    for i in range(len(predictions)):
        stars  = math.ceil((eval(reviews_list[i]["rating"]))*5)
        datum = {
            "review_text" : test_data[i],
            "confidence" : round(probabilities[i,0]*100), # ex 84
            # probabilities[i,0]  -> comp generated
    ...
    #     "cg_probabilities": list(map(lambda x: round(x * 100, 2), probabilities[:, 0])),
    #     "or_probabilities": list(map(lambda x: round(x * 100, 2), probabilities[:, 1]))
    # }
    # return response
    print(results)
    return results
...
if __name__ == "__main__":
      # uncomment reviews_list
    test_model(reviews_list)
